...
#set all settings
$AuthorizeMeObj->{'settings'}->{'token_name'} =
  'imok_token';    #will show up in cookie
$AuthorizeMeObj->{'settings'}->{'token_max-age'} =
  '3153600000';    #string time in seconds the cookie will live
...
$AuthorizeMeObj->{'settings'}->{'forgot_password_email_subject'} =
  'Password Reset - IMOK';
$AuthorizeMeObj->{'settings'}->{'Activation_Email_Subject'} =
  'IMOK account activation email';
$AuthorizeMeObj->{'settings'}->{'registration_email_template'} =
  qq(You have registered for an IMOK account.
...
$AuthorizeMeObj->{'settings'}->{'forgot_password_email_template'} =
  qq(You have requested a password recovery for an IMOK account.
    Your new password is <%set_password_code%>
    https://www.emogic.com/cgi/imok/imok.cgi
        );
my $path_to_users = $AuthorizeMeObj->{'settings'}->{'path_to_users'} =
  './users/';
$AuthorizeMeObj->{'settings'}->{'path_to_tokens'}         = './tokens/';
$AuthorizeMeObj->{'settings'}->{'path_to_authorizations'} = './authorizations/';

$AuthorizeMeObj->{'settings'}->{'max_failed_attempts'} = 10;    #password
...
sub make_email_list() {
    my $user       = shift;
    my @email_list = (
        $user->{'email'},           $user->{'email_contact_1'},
        $user->{'email_contact_2'}, $user->{'email_contact_3'}
    );
...
sub main() {
    ...
    foreach my $directory (@directories) {
        if ( -e $directory ) {
            ...
            mkdir $directory;
            &write_to_log("Missing directory $directory created");
        }
    }

    $pre_warn_email_template =
qq(Your IMOK alert will be sent soon. You should push the IMOK button at: https://www.emogic.com/cgi/imok/imok.cgi.)
      ;    #needs to be BEFORE we run cron
    if ( $ARGV[0] eq 'cron' ) {
        &cron();
        exit;
    }      #this was a cron call, so exit after processing
    ...
    if ($logged_in) {                                 #we are logged in
        ...
        if ( $command eq 'logout_all_devices' ) {
            $logged_in = &logout_all_devices();
        }
        if ( $command eq 'reset_password' ) {
            &reset_password( $in{'current_password'}, $in{'new_password'} );
        }
        ...
        if ( $command eq 'set_settings' ) {
            ...
            $user = $AuthorizeMeObj->{'user'}
              ;    #get changes that were saved. may need these below
                   #mail message
            $AuthorizeMeObj->{'settings'}->{'email_to'} = $user->{'email'};
            $AuthorizeMeObj->{'settings'}->{'email_subject'} =
              'IMOK Settings Changed';    #provide later
            $AuthorizeMeObj->{'settings'}->{'email_message'} =
"Your IMOK Settings have been changed.<br>Access site at: https://www.emogic.com/cgi/imok/imok.cgi";
        ...
        if ( $command eq 'register' ) {
            &register();
        }     #load register form from ./forms/register.html or just jump to it?
        if ( $command eq 'activate' ) {
            &activate( $in{'activate_code'}, $in{'user_id'} );
        }     #login email , password
        if ( $command eq 'login' ) {
            $logged_in = &login();
            $user      = $AuthorizeMeObj->{'user'};
        }     #login email , password
        if ( $command eq 'forgot_password' ) {
            &forgot_password( $in{'email'} );
        }
        if ( $command eq 'set_password' ) {
            &set_password( $in{'user_id'}, $in{'set_password_code'} );
        }     #from link sent by &forgot_password
    }
...
|;
    ...
    if ( ( $logged_in == 1 ) && ( $user->{'first_login'} == 1 ) )
    {    #force user to see settings page
        $output = &get_settings($user);
        $output =~ s/<%welcome%>/$welcome/g;    #first welcome for settings page
    }
    else {
        $output =~
          s/<%welcome%>//g; #blank welcome for settings page on all other visits
    }
    ...
    if ($logged_in) {    #we are logged in from cookie token or login routine
        ...
        my $filename =
          "$AuthorizeMeObj->{'settings'}->{'path_to_users'}$user->{'user_id'}";
        my $trigger_time = &get_time_stamp($filename);
        $output =~ s/<%trigger_time%>/$trigger_time/g;    #for main page  != 0

        my $check_in_date =
          scalar localtime($trigger_time);    #WRONG should be timestamp
    ...
    print $output;
    &write_to_log("Print HTML output template");
    &write_to_log("End of main()");
    &write_to_log("-----------");
}    #main done
...
sub write_to_log() {
    ...
    $year = 1900 + $year;
    my $time_string =
      sprintf( "%d-%.2d-%.2d  %d:%.2d", $year, $mon, $mday, $hour, $min );
...
sub imok() {
    ...
    my ( $dev, $ino, $mode, $nlink, $uid, $gid, $rdev, $size, $atime,
        $current_time_stamp, $ctime, $blksize, $blocks )
      = stat($filename);

    my $new_time_stamp = $current_time_stamp
      ;    #will ALWAYS jump to next start_time (after now) + timeout

    my $now = time();

    if ( $current_time_stamp <= $now ) {    #alarm was/is triggered
        until ( $new_time_stamp > $now )
        { #we do this loop as we are basing our repeating Alert date/times based on our initial setting, not based on when we click the imok button
            $new_time_stamp = $new_time_stamp + $user->{'timeout_sec'};
        }
        $message =
"$message Alarm was likely triggered. Please email your contacts and tell them you are OK.";

        #send out IMOK email. Member has checked in...
    }
    elsif ( ( $current_time_stamp - $user->{'timeout_sec'} ) <= $now )
    {    #we are clicking just before alarm is triggered
        until ( $new_time_stamp > ( $now + $user->{'timeout_sec'} ) ) {
            $new_time_stamp = $new_time_stamp + $user->{'timeout_sec'};
        }
    }
    elsif ( ( $current_time_stamp - $user->{'timeout_sec'} ) > $now )
    {    # we are a full timeout before the time stamp. do nothing
        return 1;
    }    #do nothing

    #trigger time on users computer
    my $new_time_stamp_user_tz =
      $new_time_stamp + ( $user->{'tz_offset_hours'} * 60 * 60 );
    my ( $sec, $min, $hour, $mday, $mon, $year, $wday, $yday, $isdst ) =
      gmtime($new_time_stamp_user_tz);
    $mon  = $mon + 1;
    $year = 1900 + $year;
    my $trigger_time_string =
      sprintf( "%d-%.2d-%.2d  %d:%.2d", $year, $mon, $mday, $hour, $min );
    ...
    &write_to_log("$user->{'email'} pushed IMOK.");
    return $result;
}
...
sub imnotok() {
    ...
    $imnotok_email_template =
qq($user->{'email'} has pushed the IM_NOT_OK button. Please check on them.);
    ...
    &write_to_log(
"sendmail result : $result : $user->{'email_contact_1'} : $user->{'email'}"
    );
    $message =
"$message sendmail result : $result : $user->{'email_contact_1'} : $user->{'email'} : $imnotok_email_template ";
}
...
sub testimok() {
    ...
    $test_email_template =
qq(This email was sent by the IMOK system as a test by $user->{'email'} . Please let them know you received it.);
    $AuthorizeMeObj->{'settings'}->{'email_to'}      = $email_list;
...
sub cron() {
    ...
    my @filenames =
      glob("$path_to_users*");    #get the list of files in the users directory
    foreach my $filename (@filenames) {

        &write_to_log("Looking at $filename");
        my $user =
          $AuthorizeMeObj->db_to_hash($filename);    #open file get details
        $email_list = &make_email_list($user);
        my $timestamp = &get_time_stamp($filename);

        my $t = time();
        &write_to_log(
"Time is $t and timestamp is $timestamp and pretime is $user->{'pre_warn_time'} and last email sent at $user->{'last_email_sent_at'}"
        );

        if (   ( time() > ( $timestamp - $user->{'pre_warn_time_sec'} ) )
            && ( $timestamp > time() ) )
        {                                            #send prewarn email to self
            $AuthorizeMeObj->{'settings'}->{'email_to'}      = $user->{'email'};
        ...
        $AuthorizeMeObj->{'settings'}->{'email_message'} =
"$user->{'email_form'} <p> Alert was sent on behalf of $user->{'email'} </p>"
          ; #add users email at end of message in case they do not provide any identification in the email
        my $result = $AuthorizeMeObj->email();
        &write_to_log(
"sendmail result : $result : $user->{'email_contact_1'} : $user->{'email'}"
        );
        ...
        &write_to_log(
"$filename Alert to $email_list at $t :  $AuthorizeMeObj->{'settings'}->{'email_message'}"
        );
        &write_to_log("----");
    }
...
sub time_zone() {
    ...
    my @lt = localtime( $today_seconds_at_00 + ( $day_seconds / 2 ) )
      ;    #must use noon or we will get odd values
...
sub get_settings() {    #input user, output html
    my $user   = shift;
    my $output = &get_template_page('settings.html')
      ;                 #string passed by ref so we can modify it
                        #replace tokens
...
sub set_settings() {
    ...
    if ( ( $email eq '' ) || ( $AuthorizeMeObj->valid_email($email) ) ) {
        ...
        $message =
"$message : $email is not a valid email address. Go back to settings and enter a valid email address.";
        return 0;
    }
...
"$message : $email is not a valid email address. Go back to settings and enter a valid email address.";
    ...
    $AuthorizeMeObj->{'user'} =
      $user;    #save it back to object as hash_to_db does not

    if ( $result == 1 ) {
        $message =
"$message Your Alert has been set. Verify the alert date and time. You can then close this program or leave it open. Open it again when you want to push the IMOK button.";
    }
    else {
    ...
    my $hour_seconds = 60 * 60;
    my $day_seconds  = 24 * $hour_seconds;
    my $timestamp    = $in{'timestamp'}
      ; #generated by JS on settings.html page and based on Alert time and date #Alert timestamp based on PC's local time
    $result = &set_time_stamp( $timestamp, "$path_to_users$user->{'user_id'}" );
    if ( $result == 0 ) {
        $message =
"$message Could not set timestamp on $path_to_users$user->{'user_id'}";
    }
    return $result;
}
...
sub register() {
    ...
    if ( $result == 0 ) {
        ...
        $message =
"You have registered for an IMOK account. Please check your email, and click on the link within, to activate your account.";
    }
    return $result;
}
...
sub activate() {
    ...
    if ( !defined($user) ) {
        $message = "$message Your activation failed.";
        return 0;
    }
...
');
    ...
    my $result   = $AuthorizeMeObj->hash_to_db( $user, $filename );
    &set_time_stamp( $user->{'timestamp'}, $filename );    #update time stamp

    if ( $result == 1 ) {
        $message =
"$message Your account for $user->{'email'} has been activated. Please log in and complete your setup.";
    }
    else {
        $message =
"$message Error: Your account $user->{'email'} could not be authorized";
    }
    return $result;
}
...
sub login() {
    ...
    #get file timestamp
    my $user_id = $AuthorizeMeObj->create_user_id( lc $in{'email'} )
      ;    #user id is hash of email
    my $filename   = "$AuthorizeMeObj->{'settings'}->{'path_to_users'}$user_id";
    my $time_stamp = &get_time_stamp($filename);

    my $result = $AuthorizeMeObj->login( lc $in{'email'}, $in{'password'} )
      ;    #note email coveted to lower case!

    #restore time stamp
    &set_time_stamp( $time_stamp, $filename );    #update time stamp
...
sub logout_all_devices() {
    ...
    my $user   = $AuthorizeMeObj->{'user'};
    if ( $result == 1 ) {
        $message = "$message You have logged out of all devices.";
    }
    else {
...
sub reset_password() {
    ...
    my $user_id =
      $AuthorizeMeObj->create_user_id( lc $email );    #user id is hash of email
    my $filename   = "$AuthorizeMeObj->{'settings'}->{'path_to_users'}$user_id";
    my $time_stamp = &get_time_stamp($filename);

    my $result =
      $AuthorizeMeObj->reset_password( $current_password, $new_password );

    #restore time stamp
    &set_time_stamp( $time_stamp, $filename );         #update time stamp

    if ( $result == 1 ) {
        $message = "$message Password was reset. You are no longer logged in.You must login again.";
        &logout_all_devices();
        &write_to_log("Passwword reset for $AuthorizeMeObj->{'user'}->{'user_id'}");
        &write_to_log("Logout all devices for $AuthorizeMeObj->{'user'}->{'user_id'}");
    }
    else {
        $message = "$message Password was not reset";
         &write_to_log("Passwword reset attempt for $AuthorizeMeObj->{'user'}->{'user_id'} failed");
    }
    return $result;
}
...
"This script must be called from the Web\nusing either GET or POST requests\n\n"
  ...
  PAIR: foreach $pair (@pairs) {
        ...
        if ( $value eq "---" ) {
            next PAIR;
        }    # This is used as a default choice for select lists and is ignored.
        ( exists $in{$name} )
          ? ( $in{$name} .= "~~$value" )
